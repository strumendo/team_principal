...
@pytest.mark.asyncio
async def test_list_permissions_empty(client: AsyncClient, superuser_headers: dict[str, str]) -> None:
...
@pytest.mark.asyncio
async def test_create_permission(client: AsyncClient, superuser_headers: dict[str, str]) -> None:
...
@pytest.mark.asyncio
async def test_create_permission_duplicate(client: AsyncClient, superuser_headers: dict[str, str]) -> None:
    ...
    await client.post("/api/v1/permissions/", headers=superuser_headers, json=payload)
    response = await client.post("/api/v1/permissions/", headers=superuser_headers, json=payload)
...
async def test_list_permissions_filter_by_module(
    ...
    response = await client.get("/api/v1/permissions/?module=users", headers=superuser_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
async def test_get_permission_by_id(
    client: AsyncClient, superuser_headers: dict[str, str], test_permission: Permission
) -> None:
    """Test GET /api/v1/permissions/{id} returns permission / Testa GET /permissions/{id}."""
    response = await client.get(f"/api/v1/permissions/{test_permission.id}", headers=superuser_headers)
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_get_permission_not_found(client: AsyncClient, superuser_headers: dict[str, str]) -> None:
    """Test GET /api/v1/permissions/{id} with invalid ID returns 404 / Testa com ID invalido retorna 404."""
    fake_id = uuid.uuid4()
    response = await client.get(f"/api/v1/permissions/{fake_id}", headers=superuser_headers)
